...
PUBLIC void init_screen(TTY* tty)
{
	...
	int size_per_con = v_mem_size / NR_CONSOLES; /* 每个控制台占的显存大小 */
	tty->console->orig = nr_tty * size_per_con; /* 当前控制台占的显存开始地址 */
	tty->console->con_size = size_per_con / SCR_WIDTH * SCR_WIDTH; /* 当前控制台占的显存大小*/
	/*tty->console->cursor = tty->console->crtc_start = tty->console->orig;  光标位置 当前控制台显示到了显存的什么位置 */
...
PUBLIC int sys_clear()
{
	int i;
        ...
        //printf("%d\n", con->orig);
        //printf("%d\n", V_MEM_BASE);
        //CONSOLE* con = &console_table[current_console = con_num];
	//if (is_current_console(con)) {
		//scroll_screen(con, SCR_UP);
		//clear_screen(con->cursor, con->con_size);
                //flush(con);
	//}
